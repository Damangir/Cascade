...
Although these changes are visually appreciable there is no concrete definition or gold standard, which makes segmentation and measurement a daunting task. Although many automatic methods have been proposed in the last 15 years, no single method is widely employed. Once of the reasons can be tracked in the desirables characteristics of a algorithm to be used in clinical setup \cite{GarciaReview}. Table \ref{Table:Requirements} presents the desirable characteristics of an algorithm to be widely used.
...
We believe that failure in addressing these specifications rooted in the fact that the problem of segmenting WMC as viewed today is an \textit{ill-posed} problem\cite{undefined} meaning the solution to the problem is not unique. In other words, there is no WMC
To the authors knowledge, previously presented methods aimed to produce a segmentation as close to manual segmentation, presumed as gold standard, as possible.
