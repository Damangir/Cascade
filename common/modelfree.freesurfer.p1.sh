...
cp "${FLAIR}" "${OUTPUT_DIR}/flair.coreg.nii.gz"
"${FSL_PRE}flirt" -in "${OUTPUT_DIR}/rawavg.nii.gz" -ref "${FLAIR}" -out "${OUTPUT_DIR}/t1.coreg.nii.gz" -omat "${OUTPUT_DIR}/t1.to.ref.tfm" -dof 7
"${FSL_PRE}flirt" -in "${OUTPUT_DIR}/brain.tissues.nii.gz" -ref "${FLAIR}" -out "${OUTPUT_DIR}/brain.tissues.coreg.nii.gz" -init "${OUTPUT_DIR}/t1.to.ref.tfm" -applyxfm
################################################################################
# End Patch 1: FSL Coregistration                                              #
################################################################################
...
################################################################################
"${FSL_PRE}fslmaths" "${OUTPUT_DIR}/brain.tissues.coreg.nii.gz" "-thr" "3" "-bin" "${OUTPUT_DIR}/wm_mask.nii.gz"
################################################################################
# End: Create WM mask                                                          #
################################################################################
...
################################################################################
"${FSL_PRE}fslmaths" "${OUTPUT_DIR}/brain.tissues.coreg.nii.gz" "-thr" "3" "-bin" "${OUTPUT_DIR}/wm_mask.nii.gz"
"${FSL_PRE}fslmaths" "${OUTPUT_DIR}/brain.tissues.coreg.nii.gz" "-thr" "2" "-bin" "${OUTPUT_DIR}/wg_mask.nii.gz"
"${FSL_PRE}fslmaths" "${OUTPUT_DIR}/brain.tissues.coreg.nii.gz" "-thr" "1" "-bin" "${OUTPUT_DIR}/brain_mask.nii.gz"
################################################################################
# End: Create WM mask                                                          #
################################################################################
...
"${FSL_PRE}fslmaths" "${OUTPUT_DIR}/flair.evident.nii.gz" "-bin" "-add" "${OUTPUT_DIR}/t1.evident.nii.gz" "-bin" "-mul" "-1" "-add" "1" "${OUTPUT_DIR}/evident_mask.nii.gz"
"${FSL_PRE}fslmaths" "${OUTPUT_DIR}/brain.tissues.coreg.nii.gz" "-thr" "3" "-bin" "-mul" "${OUTPUT_DIR}/evident_mask.nii.gz" "${OUTPUT_DIR}/evident_mask.nii.gz"
else
"${FSL_PRE}fslmaths" "${OUTPUT_DIR}/brain.tissues.coreg.nii.gz" "-thr" "3" "-bin" "${OUTPUT_DIR}/evident_mask.nii.gz"
fi
################################################################################
...
"${FSL_PRE}fslmaths" "${OUTPUT_DIR}/flair.modelfree.pval.nii.gz" "-max" "${OUTPUT_DIR}/t1.modelfree.pval.nii.gz" "${OUTPUT_DIR}/wml.pvalue.nii.gz"
################################################################################
