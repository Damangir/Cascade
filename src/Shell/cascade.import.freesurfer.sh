...
then
	...
	EXEC mri_convert "${FS_ASEG}" "${OUTPUT_DIR}${FS_ASEG_NII}"
	EXEC ${CASCADE_BIN}resample "${OUTPUT_DIR}${FS_T1_NII}" "${OUTPUT_DIR}${FS_ASEG_NII}" "${OUTPUT_DIR}${FS_ASEG_NII}" "${UNITY_TRANSFORM}" nn
else
	EXEC ${CASCADE_BIN}ImportImage "${FS_T1}" "${OUTPUT_DIR}${FS_T1_NII}"
	EXEC ${CASCADE_BIN}ImportImage "${FS_ASEG}" "${OUTPUT_DIR}${FS_ASEG_NII}"
	EXEC ${CASCADE_BIN}resample "${OUTPUT_DIR}${FS_T1_NII}" "${OUTPUT_DIR}${FS_ASEG_NII}" "${OUTPUT_DIR}${FS_ASEG_NII}" "${UNITY_TRANSFORM}" nn
fi
EXEC ${CASCADE_BIN}relabel "${OUTPUT_DIR}${FS_ASEG_NII}" "${FREESURFER_BTS_IMPORT}" "${OUTPUT_DIR}${BTS_MAP}"